...
let allProducts = [];
let filterText = '';
let filterTab = 'my'; // 'my' = My Choice (no external link), 'tiktok' = items with external_url
...
function renderProducts() {
  ...
  const filtered = allProducts.filter(p => {
    ...
    } else if (filterTab === 'tiktok') {
      matchTab = (p.tag || '') === 'tiktok';
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // tabs behavior: TikTok opens external shop
  document.querySelectorAll('.tab').forEach(t => {
    t.addEventListener('click', (ev) => {
      ...
      if (txt === 'tiktok') filterTab = 'tiktok';
      else filterTab = 'my';
      renderProducts();
    });
  });
  ...
  if (productForm) {
    productForm.addEventListener('submit', async (e) => {
      ...
      // client-side: ensure tiktok tag includes a link
      const tagVal = fd.get('tag') || 'mychoice';
      const ext = (fd.get('external_url') || '').toString().trim();
      if (tagVal === 'tiktok' && ext === '') {
        alert('Vui lòng nhập link bán hàng khi chọn tag TikTok');
        return;
      }
      if (editId) {
